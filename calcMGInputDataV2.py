...
def get_args():
    ...
    psr.add_argument('-iXi','--inXiFileName', default='xiData_internet_1104_1117.csv', help='File name of Input Xi data')  
    ...
    psr.add_argument("--uCPU1ave","-cave1",default=0.25, type=float, help='CPU for class 1')
    psr.add_argument("--uCPU2ave","-cave2",default=0.25, type=float, help='CPU for class 2')
    psr.add_argument("--uMEM1ave","-mave1",default=0.80, type=float, help='MEM for class 1')
    psr.add_argument("--uMEM2ave","-mave2",default=0.80, type=float, help='MEM for class 2')
    psr.add_argument("--uCPU1std","-cstd1",default=0.1, type=float, help='CPU for class 1')
    psr.add_argument("--uCPU2std","-cstd2",default=0.1, type=float, help='CPU for class 2')
    psr.add_argument("--uMEM1std","-mstd1",default=0.3, type=float, help='MEM for class 1')
    psr.add_argument("--uMEM2std","-mstd2",default=0.3, type=float, help='MEM for class 2')
...
if __name__ == '__main__':
    ...
    output = args.output
    mkdir(output)
    inputFileNameOriginalTraffic = args.output+"/"+args.inputFileNameOriginalTraffic
    input_file_name = args.output+"/"+args.fileName
    input_xi_file_name = args.output+"/"+args.inXiFileName
    ...
    resource_1 = UserTraffic(arrivalRate1,aveDataSize1,uCPU1ave,uCPU1std,uMEM1ave,uMEM1std) # arrivalRate=1.25,AveDataSize=100 -> 1Mbps?
    resource_2 = UserTraffic(arrivalRate2,aveDataSize2,uCPU2ave,uCPU2std,uMEM2ave,uMEM2std) # arrivalRate=6.25,AveDataSize=100 -> 5Mbps?
    #
