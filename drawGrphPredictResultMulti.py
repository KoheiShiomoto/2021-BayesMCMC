...
def get_args():
    ...
    psr.add_argument('--pjName', help='ProjectName')  
...
if __name__ == '__main__':
    ...
    #
    fig,ax = plt.subplots(1,1,figsize=(12,9),sharex=True,sharey=True)
    df_1['predicted-xi'] = df_1['predicted-xi1']+df_1['predicted-xi2']
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f"{picdir}/{pjName}_Multi_ActualPredictedTraffic.pdf")
    plt.show()
    #
    fig = plt.figure(figsize=(12,8))
    ax = subplot(111)
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig(f"{picdir}/{pjName}_Multi_RelativeError.pdf")
    plt.show()
